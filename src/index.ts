...
const addIndent = (string: string, indentation: number, whitespace: string = '  ') => 
    `${[...Array(indentation).keys()].map(_ => '  ').join('')}${string}`;

const indentEachLine = (string: string, indentation: number, whitespace: string = '  ') => 
    string
    .split('\n')
    .map(e => addIndent(e, indentation, whitespace))
    .join('\n')

const formatRpcs = (rpcs: string[]) => 
    rpcs
    .map(
        rpc => rpc
            .split('\n')
...
const service = (serviceName: string, rpcs: string[]) => 
`service Service {
...
const rpc = (rpcName: string, serviceName: string, requestSchemaName: string, responseSchemaName: string, route: string, method: string, visibility: string = 'internal') =>
`rpc ${rpcName}( ${requestSchemaName} ) returns( ${responseSchemaName} ) {
...
function message(messageName: string, schema, indent = 0) {
...
function addParams(fullParams: Params, properties: Params, required: boolean) {
    fullParams.properties = Object.assign(fullParams.properties, properties)
    fullParams.required = fullParams.required.concat(required ?? [])
}
...
export const plugin = fastifyPlugin(
  function (instance: FastifyInstance, options: FastifyPluginOptions, done: (error?: FastifyError) => void): void {
    ...
    instance.addHook('onRoute', route => {
      console.debug('Added route', util.inspect(route, {showHidden: false, depth: null, colors: true}));
      routes.push(route)
    })
    ...
    instance.addHook('onReady', done => {
      ...
      const serviceName = options.serviceName ?? swagger.info.title;
        ...
        try {
            ...
            rpcs.push(
                rpc(
                    ...
                    ? `${rpcName}Request`: 'google.protobuf.Empty',
                    routeSchema ? `${rpcName}Response` : 'string',
                    route.url,
